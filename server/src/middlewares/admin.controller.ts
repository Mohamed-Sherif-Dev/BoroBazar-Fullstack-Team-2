...
export const addProduct = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    // Image URL from Cloudinary if uploaded, else fallback
...
export const editProduct = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (req.file) console.log("New image file received:", (req.file as any).path);

    const updateData: any = { ...req.body };

    // Prevent unsetting name to empty
    ...
    // Convert strings to proper types (Multer FormData sends everything as strings)
    ...
    // Explicitly handle empty oldPrice if needed (e.g. to remove discount)
    if (updateData.oldPrice === "") {
        updatePayload.$unset = { oldPrice: 1 };
        delete updateData.oldPrice; // Remove from $set part
    }
